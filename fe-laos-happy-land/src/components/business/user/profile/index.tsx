...
export default function Profile() {
  ...
  useEffect(() => {
    if (userData?.user) {
      ...
      if (user.location) {
        // For now, we'll just set the address in locationData
        // Later this could be enhanced to parse JSON location data
        setLocationData({
          latitude: 0, // Default coordinates for Laos
          longitude: 0,
          address: user.location,
        });
      }
  ...
  const handleLocationChange = (location: LocationDto | null) => {
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        <MapboxLocationSelector
                          ...
                          disabled={!isEditing}
                          initialSearchValue={formData.address}
                        />
                      ) : (
                        <Input
